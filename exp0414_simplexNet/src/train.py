...
- Moving window training for sequential learning
"""
...
def train_with_window(config: Config, trace_fn=None):
    """Train model with moving window training.

    Window slides from left to right across the domain without cycling.
    Each window position is trained for `window_hold` epochs before sliding.

    Args:
        config: Configuration object with window_width and window_hold set
        trace_fn: Optional callback for tracking parameters

    Returns:
        Dictionary with training results (same structure as train_with_config)
    """
    # Set seeds
    random.seed(config.seed)
    np.random.seed(config.seed)
    ...
    # Window parameters
    window_width = config.window_width * config.domain_width  # Absolute width
    window_hold = config.window_hold
    n_window_positions = max(1, int(math.ceil((1.0 - config.window_width) * config.epochs / window_hold)))

    # Pre-compute window positions (normalized 0 to 1)
    # Window starts at position 0 and ends at position (1 - window_width)
    window_positions = np.linspace(0, 1.0 - config.window_width, n_window_positions)
    ...
    epoch = 0
    for window_idx, window_start_norm in enumerate(window_positions):
        # Compute window bounds
        window_start = config.x_min + window_start_norm * config.domain_width
        window_end = window_start + window_width

        # Generate training data for this window
        # Use more points to ensure good coverage even with small windows
        n_points_per_window = max(config.num_train, int(2500 * config.window_width))
        x_window = torch.linspace(window_start, window_end, n_points_per_window).unsqueeze(1)

        # Get target values
        from data import target_function_1d
        y_window = target_function_1d(x_window, config.task_name, config.custom_function)

        # Create data loader for this window
        train_loader = DataLoader(
            TensorDataset(x_window, y_window),
            batch_size=config.batch_size,
            shuffle=True,
        )

        # Train for window_hold epochs at this position
        for hold_epoch in range(window_hold):
