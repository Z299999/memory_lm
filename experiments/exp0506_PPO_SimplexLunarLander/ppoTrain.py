...
class ContinuousActor(nn.Module):
    ...
    def forward(self, s):
        dist = self._dist(s)
        a    = dist.sample()
        return a.squeeze(0), dist.log_prob(a).sum(-1).squeeze(0), dist.entropy().sum(-1).squeeze(0)
...
class Critic(nn.Module):
    def __init__(self, obs_dim, smn_n, smn_m, smn_activation):
        super().__init__()
        self.net = SMN(
            n=smn_n,
            m=smn_m,
    ...
    def forward(self, s):
        if s.dim() == 1:
            s = s.unsqueeze(0)
        return self.net(s).squeeze(-1).squeeze(0)
