...
**当 depth > 1 时的连接约定**：
- 层内横向边始终是位置到位置的全连接，所以相邻位置之间有 `depth^2` 条边
- 跨层边是否在 `x` 维上全连接，由 `cross_layer_mode` 决定
- 同一位置内的多个神经元（不同 `x`）之间仍然不直接相连
...
|---|-------|------------|--------|
| 4 | 1 | 10 | 26 |
| 4 | 2 | 20 | 40（shared_x） / 64（full_x） |
| 4 | d | 10×d | 6d² + 20d（shared_x） / 18d² + 8d（full_x） |

边数组成（L=4, depth=d）：
- 层内边（Y 方向）：`d² × (1+2+3) = 6d²`
- 下→上边（Z+）：`6d`（shared_x） / `6d²`（full_x）
- 上→下边（Z-）：`6d`（shared_x） / `6d²`（full_x）
- 输入边：`4d`
- 输出边：`4d`
- 总计：`6d² + 20d`（shared_x） / `18d² + 8d`（full_x）
...
每个位置有多个神经元。层内横向连接总是全连接，跨层连接是否全连接取决于 `cross_layer_mode`：

```text
位置 (z, y) 内部：神经元 (1, y, z), (2, y, z), ..., (d, y, z)
层内连接（Y 方向）：(*, y, z)   → (*, y+1, z)   全连接 d² 条边
跨层连接（shared_x）：(x, y, z) → (x, y', z')   每个位置对位置是 d 条边
跨层连接（full_x）：  (*, y, z) → (*, y', z')   每个位置对位置是 d² 条边
```
...
```text
exp0410_triangleNet/runs/YYYYMMDD/<run_name or task_name>_YYYYMMDD_HHMMSS/comparison_4panel.png
```
...
## 重要改进方向：晶格连接（Lattice Connections）

**当前实现的局限性**：

当前代码中的连接方式（`full_x` 模式）在层内、Z+、Z- 方向上都采用**全连接**：
- 层内：相邻位置的所有 x 之间全连 `(*, y, z) → (*, y+1, z)`
- Z+：同 y 位置的所有 x 之间全连 `(*, y, z) → (*, y, z+1)`
- Z-：对角位置的所有 x 之间全连 `(*, y, z) → (*, y+1, z-1)`

这种全连接方式**只在四面体结构中能正确工作**，但不符合标准的**二维三角晶格**结构。

**晶格连接规则**（计划中的改进）：

在标准三角晶格中，每个节点 `(x, y, z)` 只有**三个固定方向**的连接：

| 方向 | 连接规则 | 说明 |
|------|---------|------|
| 层内（左→右） | `(x,y,z) → (x,y+1,z)` | 相同 x，相邻 y |
| Z+ 垂直（下→上） | `(x,y,z) → (x,y,z+1)` | 相同 x，相同 y |
| Z- 对角（左上→右下） | `(x,y,z) → (x,y+1,z-1)` | 相同 x，对角向下 |
...
**实现状态**：
- 当前版本（exp0410）**不实现**晶格连接
- 晶格连接将在**独立的后续项目**中实现
- 本项目的代码和实验结果作为 baseline 保留
...
- `cross_layer_mode: shared_x`
- `cross_layer_mode: full_x`
...
所以当 `depth > 1` 时，`full_x` 会把这两种跨层位置到位置连接从 `depth` 条边提升到 `depth^2` 条边。
当 `depth = 1` 时，`shared_x` 和 `full_x` 会退化成相同图结构。
