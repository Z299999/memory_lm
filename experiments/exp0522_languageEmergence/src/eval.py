...
def _evaluate_rollout(
    ...
    *,
    ...
    return {
        ...
        "start_step": int(target_bundle.get("global_start_step", torch.tensor(start_step)).item()),
    ...
    *,
    ...
    return {
        "config": {
            ...
            "market_passes": config.market_passes,
        ...
        "training": {
            ...
            "final_heldout_mse": float(full_history[-1]["heldout_loss"]) if full_history else None,
        },
        "eval_results": {
            condition: {
...
def evaluate_model(config: ExperimentConfig, run_dir: Path) -> dict[str, Any]:
    ...
    common_kwargs: dict[str, Any] = dict(
        ...
        target_split="test" if config.target_kind == "yfinance_series" else "train",
        ticker=config.ticker,
        price_column=config.price_column,
        series_kind=config.series_kind,
        normalize=config.normalize,
        ...
        for condition in config.eval_conditions:
            ...
            if has_train_state:
                ...
                continuous_evals[condition] = _run_condition(
                    ...
                    start_step=0 if config.target_kind == "yfinance_series" else continuous_start_step,
