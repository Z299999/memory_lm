...
|----|------|------|----------|----------|------|
| **X 轴** | 里→外 | 深度 (Depth) | `x` | `1` 到 `depth[z]` | `x=1` 是最里层，`x=2` 是外层 |
| **Y 轴** | 左→右 | 位置 (Position) | `y` | `1` 到 `z` | `y=1` 是最左，`y=z` 是最右 |
| **Z 轴** | 下→上 | 层 (Layer) | `z` | `1` 到 `L` | `z=1` 是底层，`z=L` 是顶层 |

**节点记法**：`(x, y, z)` = `(深度，位置，层)`

**节点示例**（L=4, depth=[2,2,2,2]）：
- `(2, 1, 4)` = 外层，左边，从下往上数第 4 层（左上角外层）
- `(1, 1, 4)` = 里层，左边，从下往上数第 4 层（左上角里层）
- `(2, 4, 1)` = 外层，最右，底层（右下角外层）
- `(1, 1, 1)` = 里层，最左，底层（左下角里层）
...
| 类型 | 方向 | 规则 | 示例 |
|------|------|------|------|
| **层内连接** | Y 方向（左→右） | 相邻位置之间全连接 | `(2, 1, 4) → (1, 2, 4)` |
| **跨层连接** | Z 方向（下→上 / 上→下） | 上层→下层（保持三角结构） | `(2, 1, 4) → (1, 1, 3)` |

**连出/连入约定**：
- **连出**：从源位置的**最大 X**（最外层，`x = depth[z]`）
...
- 层内边（Y 方向）：`(L-1) × d² + (L-2) × d² + ... = d² × L(L-1)/2`
- 跨层边（Z 方向）：原始三角边数 × d²
- 总计：约 72 条边（原始 15 条）
...
```text
位置 (z, y) 内部：神经元 (1, y, z), (2, y, z), ..., (depth[z], y, z)
层内连接（Y 方向）：(depth[z], y, z) → (1, y+1, z)  全连接 d² 条边
跨层连接（Z 方向）：(depth[z], y, z) → (1, y', z-1) 全连接 d² 条边
```
