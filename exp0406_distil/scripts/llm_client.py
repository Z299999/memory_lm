"""LLM Client for calling teacher model API."""
...
class OpenAICompatClient:
    """OpenAI-compatible API client for DashScope and other providers."""

    def __init__(
        self,
        api_key: str | None = None,
        base_url: str | None = None,
        timeout: int = 300,  # Increased to 5 minutes for slow responses
        stub: bool = False,
    ) -> None:
        load_repo_dotenv()
        ...
        if not self.stub and not self.api_key:
            raise LLMClientError(
                "Missing API key. Set DASHSCOPE_API_KEY in the environment or repo .env."
            )
